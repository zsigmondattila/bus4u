...
if (props.bus) {
  axios.get('/v1/admin/get_bus_by_id', { params: { bus_uid: props.bus.toUpperCase() } })
    .then(rsp => {
      form.value = rsp.data
...
async function onSubmit(event) {
  ...
  if (props.bus) {
    ...
    axios.put('/v1/admin/update_bus', data, { params: { bus_uid: props.bus.toUpperCase() } })
      .then((rsp) => {
        if (rsp.status === 200) {
    ...
    axios.post('/v1/admin/create_bus', form.value).then((rsp) => {
      if (rsp.status === 200) {
        router.replace({ name: 'buses' })
      } else {
        isLoading.value = false

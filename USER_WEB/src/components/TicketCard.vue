  ...
  <v-card height="100%" class="mx-auto d-flex flex-column" min-width="150" max-width="300" color="orange-lighten-5">
    <v-responsive aspect-ratio="1">
      <v-sheet class="text-center h-100">
        <v-img v-if="qr" :src="qr"></v-img>
        <div v-else class="h-100 d-flex justify-center">
          <v-icon size="150" icon="mdi-ticket-confirmation-outline"
...
onMounted(() => {
  if (!props.ticket.is_valid) return
  if (props.ticket.qr) {
    qr.value = props.ticket.qr
    return
  }
  QRCode.toDataURL(props.ticket.ticket_uid, { width: 250 }).then(rsp => qr.value = rsp)
})
</script>

...
async function getCities() {
  cities.value = (await axios.get('/v1/get_cities')).data.cities
}

async function getStations(city) {
  form.station = null
  form.route = null
  if (city) stations.value = (await axios.get('/v1/get_stations_by_city', { params: { city_uid: city.city_uid } })).data.stations
}

async function getRoutes(station) {
  form.route = ''
  if (station) routes.value = (await axios.get('/v1/get_routes_by_station', { params: { station_uid: station.station_uid } })).data.routes
}
...
async function onSubmit(e) {
  ...
  try {
    timetable.value = (await axios.get('/v1/get_departure_times_for_station_in_route', { params: { station_uid: form.station.station_uid, route_uid: form.route.route_uid } })).data
  ...
  route.value = (await axios.get('/v1/get_stations_of_a_route', { params: { route_uid: form.route.route_uid } })).data.stations

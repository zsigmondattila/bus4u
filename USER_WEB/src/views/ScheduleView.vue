<template>
  <AppLayout>
    ...
    <v-form @submit.prevent="onSubmit" validate-on="submit" class="my-10">
      <v-container class="px-0">
        <v-row justify="center">
          <v-col cols="12" sm="6">
            <v-select :items="stations" :item-props="getName" label="Station" :loading="!stations.length" v-model="form.station" class="text-field" hide-details="auto" :rules="rules" @update:model-value="getBuses"></v-select>
          </v-col>
          <v-col cols="12" sm="6">
    ...
    <Map v-if="route" :coordinate-array="route.coordinates"/>
  </AppLayout>
</template>
...
async function getStations() {
  stations.value = (await axios.get('https://bus4u.fast-table.com/v1/get_stations')).data.stations
...
async function onSubmit() {
  console.log(form);
  // route.value = (await axios.get('https://bus4u.fast-table.com/v1/get_route .......', { params:{ station_uid: from.station.station_uid, route_uid: form.bus.route_uid }})).data.route
}

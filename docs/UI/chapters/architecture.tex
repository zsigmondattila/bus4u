...
A bejelentkezést igénylő funkciók API végpontjaihoz azonosítás szükséges, amelyet a frontend a felhasználó által megadott email cím és jelszó segítségével kér le a backendtől. Ez egy JWT\footnote{JSON Web Token} token formájában történik, amelyet a frontend elment és minden további kérésben elküld a backendnek. Erre egy példa a \ref{fig:jwt} ábrán látható, ahol az adminisztrátorok számára megjelenített statisztikát kéri le a rendszer a backendtől. Ez a kérés egy Authorization fejléccel van ellátva, amely tartalmazza a JWT tokent és amely alapján azonosítódik a bejelentkezett adminisztrátor.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/jwt.png}
